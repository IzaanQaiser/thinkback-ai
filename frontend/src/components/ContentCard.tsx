...
function getPlatformIcon(platform?: string, theme?: string) {
  ...
  switch (platform) {
    ...
    case 'YouTube Shorts':
      return (
        <span className="inline-flex items-center justify-center rounded-full p-1 bg-white/60 dark:bg-dark-900/60 shadow-md" style={{ width: size + 8, height: size + 8 }}>
          <img
...
const ContentCard: React.FC<ContentCardProps> = ({ id, title, url, notes, summary, tags, favorite, createdAt, category, thumbnail, platform }) => {
  ...
  function getPlatformIconOverlay(platform?: string, theme?: string) {
    ...
    switch (platform) {
      case 'YouTube Video':
        return <span className={baseClass}><FaYoutube size={size} className="text-black dark:text-white" title="YouTube" /></span>;
      case 'YouTube Shorts':
        return (
      ...
      case 'Reddit Post':
        return <span className={baseClass}><FaReddit size={size} className="text-orange-500" title="Reddit" /></span>;
      case 'Twitter/X Post':
        return <span className={baseClass}><FaTwitter size={size} className="text-sky-500" title="Twitter/X" /></span>;
      case 'Instagram Reel':
      case 'Instagram Post':
        return <span className={baseClass}><FaInstagram size={size} className="text-pink-500" title="Instagram" /></span>;
      case 'TikTok Video':
        return <span className={baseClass}><FaTiktok size={size} className="text-black dark:text-white" title="TikTok" /></span>;
      case 'LinkedIn Post':
      case 'LinkedIn Job':
