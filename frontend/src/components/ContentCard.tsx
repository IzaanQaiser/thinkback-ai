...
const ContentCard: React.FC<ContentCardProps> = ({ id, title, url, notes, tags, favorite, createdAt, category_ids }) => {
  const category = category_ids && category_ids.length > 0 ? category_ids[0] : 'Unknown';
  return (
    <Link
      to={`/view/${id}`}
    ...
    >
      <div className="flex items-start space-x-4">
        <div className="mt-1">
          {favorite && <Star className="w-5 h-5 text-yellow-400" />}
        </div>
        ...
        <div className="flex items-center space-x-2 text-xs text-dark-500 dark:text-dark-400">
          ...
          {tags && tags.length > 0 && (
            <span className="ml-2 flex flex-wrap gap-1">
              {tags.map((tag, idx) => (
                <span key={idx} className="px-2 py-0.5 bg-primary-100/60 dark:bg-dark-800/60 text-primary-800 dark:text-primary-300 rounded-full text-xs font-semibold border border-primary-200/50 dark:border-dark-700/80">#{tag}</span>
              ))}
            </span>
          )}
        </div>
